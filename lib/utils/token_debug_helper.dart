...
class TokenDebugHelper {
  ...
  static Future<Map<String, dynamic>> testTokenValidity(String accessToken) async {
    try {
      print('🔍 [TokenDebug] Testing token validity...');
      print('🔍 [TokenDebug] Token length: ${accessToken.length}');
      print('🔍 [TokenDebug] Token preview: ${accessToken.substring(0, 10)}...');
      ...
      if (response.statusCode == 200) {
        ...
        print('✅ [TokenDebug] Token is valid! User: ${data['display_name']} (${data['id']})');
        return {
          'valid': true,
          'user': data,
        ...
        if (response.statusCode == 401) {
          ...
          if (response.body.contains('Check settings on developer.spotify.com/dashboard')) {
            ...
            errorMessage = 'Access forbidden (403). Check app permissions and scopes';
          }
  ...
  static Map<String, dynamic> testEnvironmentConfig() {
    ...
    print('🔍 [TokenDebug] Client ID: ${clientId.isNotEmpty ? "✅ Set (${clientId.length} chars)" : "❌ Missing"}');
    print('🔍 [TokenDebug] Client Secret: ${clientSecret.isNotEmpty ? "✅ Set (${clientSecret.length} chars)" : "❌ Missing"}');
    print('🔍 [TokenDebug] Redirect URI: ${redirectUri.isNotEmpty ? "✅ $redirectUri" : "❌ Missing"}');
    print('🔍 [TokenDebug] API Base URL: ${apiBaseUrl.isNotEmpty ? "✅ $apiBaseUrl" : "❌ Missing"}');
    print('🔍 [TokenDebug] Auth URL: ${authUrl.isNotEmpty ? "✅ $authUrl" : "❌ Missing"}');
    ...
    return {
      ...
      'authUrl': authUrl,
      'allConfigured': clientId.isNotEmpty && clientSecret.isNotEmpty && redirectUri.isNotEmpty
    };
  }
